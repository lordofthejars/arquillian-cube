...
tomcat: # <1>
  ...
  attachStdin: true
  attachStderr: true
  env: [TOMCAT_PASS=mypass, JAVA_OPTS=-Dcom.sun.management.jmxremote.bindPort=8089] # <6>
  cmd: [] # <7>
  dns: [127.0.0.1]
  volumes: [/tmp]
  volumesFrom: [tomcat]
...
<8> Port forwarding is configured using `portBinding` section. It contains a list of `exposedPort` and `bindPort` separated by arrow (_->_). If only one bindPort is provided, *Arquillian Cube* will expose the same bindPort number. In this example the exposed bindPort 8089 is mapped to 8089 and exposed bindPort 8080 is mapped to 8081.
...
log:: it looking for a specified pattern in container log to detect service startup. This can be used when there is no bindPort to connect or connecting to the bindPort successfully doesn't mean the service is fully initialized.
...
On the other side +portBindings+ are a way to instruct _Docker_ container to publish a bindPort to be available from outside (for example from our test).

It seems reasonable that if you set a bindPort binding this bindPort should automatically be exposed bindPort as well.
For this reason in *Arquillian Cube* you can use +portBinding+ and it will automatically set to exposed bindPort.

In next example we are only setting +portBinding+ and *Arquillian Cube* will instruct _Docker_ to expose bindPort 8080 and of course bind the bindPort 8080 so it can be accessible from outside.
