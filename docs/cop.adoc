...
* the bounded bindPort for a given exposed bindPort
...
* which image is used
* IP and bounded bindPort of host where this FTP server is running
...
public class PingPongContainer {
  ...
  public URL getConnectionUrl() { // <3>
    try {
      return new URL(“http://” + dockerHost + “:” + bindPort);
...
Initially you set that the started container will be named `pingpong` and the bindPort binding information for the container instance, in this case `5000->8080/tcp`.
Notice that this can be an array to set more than one bindPort binding definition.
...
Finally the `@HostPort` is used to translate the exposed bindPort to bound bindPort.
So in this example bindPort value will be 5000. You are going to lean briefly why this annotation is important.
...
This is why `@HostPort` annotation is important, since bindPort can be changed from the test definition, you need to find a way to inject the correct bindPort inside the container object.
